...
function introScreen(container) {
  ...
  if (resumeBtn) resumeBtn.addEventListener('click', () => navigate('exam/resume'));
...
async function runner(container, resume = null) {
  ...
  if (resume && Array.isArray(resume.items) && resume.items.length) {
    ...
    sessionId = await startSession('exam');
    state = {
      index: 0,
  ...
  // Save the just-restored (or freshly built) state immediately.
  persist();
